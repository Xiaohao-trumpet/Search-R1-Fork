...
> 原始日志：`/mnt/backup1/lgc/search-r1-data/exp_logs/{baseline,reward,algo,both}.log`；
> 逐步指标解析：`run_scripts/parse_metrics.py`；2×2 汇总：`run_scripts/build_report.py`；
> 完整实验流水账（含中途踩坑）：`exp_logs/FINDINGS.md`。
> 配置：Qwen2.5-1.5B-Instruct，NQ，10 文档 e5 小库，2 GPU + 全量 FSDP offload，
> 组大小 n_agent=5，train_batch=8，max_turns=3，25 步，单 seed。

### 11.0 一个关键的中途发现（值得写进报告）
最初用 **0.5B** 模型时，**EM 和 F1-shaped 的 reward 全为 0**（`nonzero_frac=0`、
`nonuniform_group_frac=0`、`pg_loss=0`）。debug 样本显示模型输出 `valid_format=False`、
答案是"and"这类垃圾。→ **基座策略太弱时，再好的奖励也没有信号**，这本身量化了 P1 的严重性。
两个据此做出的修正：(1) 硬格式闸门 → **软闸门**（非法格式仍得 `F1×0.1`，让部分梯度流出，
但仍不可刷——奖励始终挂在真实 F1/检索上）；(2) 换 **1.5B**（推理 demo 已证明它 untrained
就能产出合法格式+正确答案）。换完之后 reward 才非零，H1/H4 才可测。

### 11.1 2×2 主结果（25 步均值）
| 单元格（奖励 + 算法） | nonzero_frac | **nonuniform_group_frac** | kept_frac | entropy | grad_norm | resp_len | val(n=8) |
|---|---|---|---|---|---|---|---|
...
其中 `nonuniform_group_frac` = 有非零优势（产生梯度）的 GRPO 组占比，是核心指标。

### 11.2 H1 确认：奖励稠密化是第一性的杠杆
F1-shaped 相比二值 EM，把"能产生梯度的组"占比 **从 0.109 提到 0.359（3.3×）**，
`nonzero_frac` 提高 4.3×。即：**二值 EM 让 ~89% 的组因全同分而零梯度被浪费；F1 把浪费降到 ~64%。**
（注意 `reward/mean` 反而是 F1 更低——因为 EM 偶尔给满分 1.0、F1 频繁给小额部分分；
所以看梯度覆盖要用 `nonzero_frac`/`nonuniform_group_frac`，不是 `reward/mean`。）

### 11.3 H4 修正：不是"替代"，而是"奖励主导 + 算法弱互补"
- 主效应：奖励 **+0.250**，算法 **+0.047**，交互 **≈0**（近似可加，而非替代）。
- **算法 bundle 单独在稀疏 EM 上几乎不涨**（EM+A+ 0.156 vs baseline 0.109；val 甚至掉到 0.125）。
  原因看 `kept_frac`：EM 下只有 **15.6%** 的组有信息，动态采样把 84% 的 batch 丢掉、用那 16%
  的 6.4 倍复制来填——它能**放大**弱信号，但**造不出**信号；F1 下 40.6% 有信息，才有料可采。
  → **动态采样以"奖励已能制造组内方差"为前提，奖励稠密化是它的前置条件，不是它的替代品。**
- 二者**可叠加**：F1+A+ 在所有集中度指标上最好（nonuniform 0.406、最短响应 198、最低熵 0.838）。

> **修正后的报告主线**：*Outcome-only 二值 EM 让 GRPO 挨饿（89% 的组零梯度）。
> 奖励稠密化是一阶修复（梯度覆盖 3.3×）；DAPO 式算法技巧是二阶的，只有当奖励先提供了
> 组内方差、给它们东西可利用时才见效。* —— 这是一个比原假设"替代"更精确、也更有信息量的结论。

### 11.4 行为与稳定性观察
- **响应变短**：403 → 316 → 198（叠加稠密奖励+答案上限+搜索惩罚 → 策略更简洁）。
- **grad_norm 上升**（1.63 → ~3.8–4.0）：Dr.GRPO 去掉 ÷std → 优势更大 → 梯度更大；长程训练可能要调 LR。
- **熵**：F1+A+ 降得最多（0.838），25 步内仍为正（未坍缩），但这是长跑时最该盯"熵坍缩"的单元格——
  clip-higher / entropy_coeff 的保护在更长训练里才会真正吃重。
- **搜索次数**：F1 变体更少（0.744 → 0.455 → 0.334）。小库下多搜也没用，但真实语料上
  搜索惩罚系数需要调，以免"欠搜索"（已在 §5.1 标注为可调旋钮）。

### 11.5 诚实的局限
- val EM 只有 n=8 → 太粗，分不开变体（当 sanity 用，别当性能结论）。
- 25 步、单 seed、10 文档库 → 这些是**训练动态趋势**，不是收敛后的任务性能；密度/覆盖类指标是
  稳健信号，性能级结论需要更大 val 集、更多步、多 seed。
- A+ 把 3 个改动打包 → 内部无法归因。见 §11.6 组件消融。

### 11.6 A+ 组件消融（都在 F1 奖励上；端点为 none=reward 与 all=both）
| 变体（F1 +） | nonzero | nonunif | kept | entropy | **grad_norm** | resp_len | search | val(n=8) |
|---|---|---|---|---|---|---|---|---|
| none（F1+vanilla）   | 0.174 | 0.359 | — | 1.118 | 3.46  | 316 | 0.455 | 0.25 |
| +仅 Dr.GRPO          | 0.211 | **0.422** | — | 1.171 | **1.04**  | 249 | 0.335 | 0.125 |
| +仅 clip-higher      | 0.199 | 0.406 | — | 1.135 | 4.37  | 292 | 0.468 | 0.125 |
| +仅 动态采样         | 0.207 | 0.391 | 0.391 | **0.812** | **10.43** | 195 | 0.285 | 0.25 |
| +全部三个（A+）      | 0.220 | 0.406 | 0.406 | 0.838 | 4.02  | 198 | 0.334 | 0.25 |

**归因结论（关键看 grad_norm 这一列）**：
- **梯度覆盖（nonuniform）**：三个分量单独都把它从 0.359 抬到 ~0.39–0.42（Dr.GRPO 最高 0.422），
  但幅度都不大、且叠加不超过最好的单项 → 覆盖率**已饱和**，它不是算法侧的主杠杆（覆盖是奖励的活，见 §11.2）。
- **梯度幅度（grad_norm）才是分水岭**：
  - **Dr.GRPO 让梯度变小**（3.46→1.04）。去掉 ÷std 消除了低方差（近二值）组的优势膨胀
    （rewards∈{0,小} 时 std<1，除以它会放大优势；去掉后优势回归温和）→ **稳定器**，性价比最高。
  - **动态采样让梯度爆炸**（→10.43），熵降最多（0.812）、响应最短（195）：它把少数有信息的组
    复制填满 batch，更新被一小撮高优势样本主导 → 强，但**单独用有风险**（grad_norm 10 → 长跑易不稳）。
  - **clip-higher**：25 步内各项都温和（预期——此时还没发生熵坍缩，它的保护作用还没被触发）。
- **真实互补（Dr.GRPO × 动态采样）**：合在一起（A+）时，Dr.GRPO 的"收缩"抵消了动态采样的"爆炸"
  → grad_norm 10.43→4.02、熵 0.812→0.838（略安全），同时保留覆盖/简洁收益。
  → 正确搭配是 **动态采样（放大器）+ Dr.GRPO（稳定器）**；不配 Dr.GRPO 单开动态采样会梯度不稳。

**给报告的可落地结论**：
1. **奖励稠密化是一阶修复**（恢复梯度覆盖 3.3×），先做。
2. 在稠密奖励之上，**Dr.GRPO 是最安全的单一算法项**（覆盖最好且降 grad_norm→更稳），性价比最高。
3. **动态采样很猛但必须配 Dr.GRPO**（或更小 LR / grad-clip）压住组复制带来的梯度放大。
4. **clip-higher 要更长训练**（真正出现熵坍缩时）才见效，25 步内看不出。

### 11.7 更长 + 更高分辨率 val 的确认实验（EM+vanilla vs F1+A+，完整 50 步）
50 步，val 用 **n=40**（分辨率 0.025，之前 n=8 太粗）。（首次运行曾被一次会话重启打断；此处为
detached 重跑的**完整 50 步**结果。）

**val EM 轨迹（n=40）：**
| step | 0 | 10 | 20 | 30 | 40 | 50 | best |
|---|---|---|---|---|---|---|---|
| EM+vanilla | 0.050 | 0.050 | 0.050 | 0.075 | 0.100 | 0.075 | 0.100 |
| F1+A+      | 0.050 | 0.100 | 0.050 | 0.125 | 0.175 | 0.125 | 0.175 |

**50 步均值：**
| 指标 | EM+vanilla | F1+A+ | 倍数 |
|---|---|---|---|
| nonuniform_group_frac | 0.110 | 0.390 | 3.5× |
| nonzero_frac | 0.039 | 0.210 | 5.4× |
| val EM best / final | 0.100 / 0.075 | 0.175 / 0.125 | ~1.7× |
| entropy | 1.625 | 1.042 | 未坍缩 |
| grad_norm | 1.53 | 2.43 | 受控 |
| response_length | 349 | 197 | 更简洁 |

→ 跑满 50 步，F1+A+ 梯度覆盖 **3.5×**（与 25 步 2×2 的 3.3× 高度一致 → 结论稳健）、**val EM ~1.7×**
（best 0.175 vs 0.100）；**熵到 50 步仍 1.042、未坍缩**，grad_norm 2.43 受控（vs 动态采样单开 10.43，
Dr.GRPO 的稳定作用在长跑确认有效）。**这解决了 §11.5 的"val 太粗"局限：梯度覆盖优势（H1）确实转化为性能，
且长训练下稳定。**
