...
### 11.7 更长 + 更高分辨率 val 的确认实验（EM+vanilla vs F1+A+）
50 步，val 用 **n=40**（分辨率 0.025，之前 n=8 太粗）。注：首次运行在 ~step 20–35 被一次
**会话重启**打断，下表来自 partial 日志（`long_*.partial.log`），完整 50 步 detached 重跑进行中。

**val EM 轨迹（n=40）——训练动态优势确实转化为任务性能：**
...
partial 均值：nonuniform_group_frac 0.144（baseline）vs 0.382（F1+A+，2.6×，与 25 步 2×2 一致）；
响应长度 344 vs 148；**熵 1.250 vs 0.887——F1+A+ 到 step ~35 仍为正、未坍缩**（A+ 的 clip-higher
+ 熵保护在更长训练里稳住了）。

→ **一旦 val 集足够大，F1+A+ 的 val EM 约为 baseline 的 2×（0.150 vs 0.075）且仍在上升，
baseline 只 +0.025 就停滞。** 这解决了 §11.5 的"val 太粗"局限：梯度覆盖优势（H1）确实转化为性能。
（完整 50 步数字待 detached 重跑结束后补入。）
