...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange=None,
    ...
    Args:
        ...
        eos_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        cliprange: (float)
            Symmetric PPO clip range. See https://arxiv.org/abs/1707.06347
        cliprange_low / cliprange_high: (float, optional)
            Decoupled clip bounds (DAPO "clip-higher", 2503.14476). Raising only
            the upper bound (e.g. low=0.2, high=0.28) gives low-probability but
            positive-advantage ("exploration") tokens room to grow instead of
            being capped, mitigating entropy collapse. Fall back to `cliprange`
            when not set. See IMPROVEMENT_DESIGN_zh.md sec 4.2 / 5.2.
