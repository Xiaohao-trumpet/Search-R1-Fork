...
## 4.4 确认：训练动态优势真的转化成了 val EM（更高分辨率 val, n=40，完整 50 步）
之前 val 只有 n=8 太粗、分不开变体。用 n=40、跑满 50 步重跑 `EM+vanilla` vs `F1+A+`：
...
**50 步均值对比（稳健、与 25 步 2×2 一致）：**
| 指标（50 步均值） | EM+vanilla | F1+A+ | 倍数 |
|---|---|---|---|
| **nonuniform_group_frac**（有梯度的组占比） | 0.110 | **0.390** | **3.5×** |
| nonzero_frac | 0.039 | 0.210 | 5.4× |
| **val EM（best / final）** | 0.100 / 0.075 | **0.175 / 0.125** | ~1.7× |
| entropy（熵） | 1.625 | 1.042 | **未坍缩** |
| grad_norm | 1.53 | 2.43 | 受控（Dr.GRPO 压住了动态采样） |
| response_length | 349 | 197 | 更简洁 |

**结论**：跑满 50 步后，F1+A+ 的梯度覆盖 **3.5×**（与 25 步的 3.3× 高度一致，说明结论稳健）、
**val EM ~1.7×**（best 0.175 vs 0.100）；且**熵到 50 步仍为 1.042、未坍缩**，grad_norm 2.43 受控
（对比消融里动态采样单开的 10.43——Dr.GRPO 的稳定作用在长跑里确认有效）。
→ **H1 的梯度覆盖优势确实转化为性能，且在更长训练下依然稳定。**
